...
contract Donation {
    ...
    function withdraw(uint project_ID,string memory project_name, string memory to, string memory descrption, uint amount) public{
        require(fundraisers[msg.sender].status == true );
        require(project_data[project_ID].current_Amount >= amount);
    ...
    function get_withdraw_data(uint index) public view returns(uint,string memory,address,string memory,string memory,uint,uint256){
        return(
        withdraw_data[index].project_ID,
        withdraw_data[index].project_name,
        withdraw_data[index].executor,
        withdraw_data[index].to_organization,
        withdraw_data[index].descrption,
        withdraw_data[index].withdraw_amount,
        withdraw_data[index].withdraw_date);
    }
    ...
    function get_project_array()public view returns( project_struct[] memory){
        return project_data;
    }
    function get_fundraiser_pro_array()public view returns( project_struct[] memory){
        return  fundraisers[msg.sender].project_data;
    }
    function get_withdraw_array()public view returns( withdraw_struct[] memory){
